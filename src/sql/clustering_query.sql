...
--    'Clustering' as type,
    ...
    'FlyBase:'||bs.uniquename as associated_sample_or_assay_for_clustering
FROM library l
...
-- samples and assays
JOIN library_relationship lr2 ON (lr2.subject_id = l.library_id AND lr2.type_id IN (SELECT cvterm_id FROM cvterm 
WHERE name IN ('analysis_of', 'derived_analysis_of')))
JOIN library bs ON (bs.library_id = lr2.object_id AND bs.type_id IN (SELECT cvterm_id FROM cvterm WHERE name IN ('biosample', 'assay')))
